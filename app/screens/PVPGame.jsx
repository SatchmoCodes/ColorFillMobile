import { View, Text, StyleSheet, FlatList, Dimensions, Pressable, Modal, Animated, Easing, ActivityIndicator } from 'react-native'
...
const PVPGame = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    async function initialLoad() {
      ...
      if (docId != undefined && !boardLoaded) {
        ...
        if (docSnap.exists()) {
            ...
            setOpponentSelectedColor(colors[tempSquareArr[tempSquareArr.length - 1].colorIndex])
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(docRef, async (doc) => {
            if (doc.exists() && completeRef.current == null) {
                ...
                if (doc.data().turn == 'Owner' && userNameRef.current == doc.data().ownerName) {
                ...
                else if (doc.data().turn == 'Opponent' && userNameRef.current == doc.data().opponentName) {
                    let animatedValues = JSON.parse(doc.data().animationIndex)
                    animatedValues.forEach((val) => {
                        const growAnimation = Animated.timing(squareAnimArr[val], {
                            toValue: 1, 
                ...
                if (doc.data().ownerScore > Math.floor(tempSquareArr.length / 2)) {
                    ...
                    remainingSquares = (boardState.length) - (doc.data().ownerScore + doc.data().opponentScore)
                if (doc.data().turn == 'Owner' && userNameRef.current == doc.data().opponentName) {
                    setRadar(true)
                    console.log('checking radar')
                }
                else if (doc.data().turn == 'Opponent' && userNameRef.current == doc.data().ownerName) {
                    setRadar(true)
                    console.log('checking radar')
                }
                }
    ...
    async function handleComplete() {
        ...
        const ownerQuery = query(collection(db, 'Users'), where("username", "==", ownerNameVar))
        const opponentQuery = query(collection(db, 'Users'), where("username", "==", opponentNameVar))
        ...
        if (userNameRef.current == ownerNameVar) {
            if (winner == 'Owner') {
                ...
                if (ownerSnapshot.docs[0].data().currentWinStreak + 1 > ownerSnapshot.docs[0].data().bestWinStreak) {
            ...
            if (winner == 'Opponent') {
                ...
                if (opponentSnapshot.docs[0].data().currentWinStreak + 1 > opponentSnapshot.docs[0].data().bestWinStreak) {
  ...
  return (
    <View style={[styles.container, {backgroundColor: colorTheme.background}]}>
        ...
        <View style={styles.top}>
            ...
            <View style={[styles.colorRow, turn == 'Owner' && {opacity: .25}]}>
                <Pressable style={[styles.color, {backgroundColor: opponentSelectedColor == colors[0] ? 'gray' : colorOption[0]}, {opacity: opponentSelectedColor == colors[0] ? 0.25 : 1}]} onPress={() => turn == 'Opponent' && userName == opponentName && colorChange(colorOption[0])}></Pressable>
                <Pressable style={[styles.color, {backgroundColor: opponentSelectedColor == colors[1] ? 'gray' : colorOption[1]}, {opacity: opponentSelectedColor == colors[1] ? 0.25 : 1}]} onPress={() => turn == 'Opponent' && userName == opponentName && colorChange(colorOption[1])}></Pressable>
                <Pressable style={[styles.color, {backgroundColor: opponentSelectedColor == colors[2] ? 'gray' : colorOption[2]}, {opacity: opponentSelectedColor == colors[2] ? 0.25 : 1}]} onPress={() => turn == 'Opponent' && userName == opponentName && colorChange(colorOption[2])}></Pressable>
                <Pressable style={[styles.color, {backgroundColor: opponentSelectedColor == colors[3] ? 'gray' : colorOption[3]}, {opacity: opponentSelectedColor == colors[3] ? 0.25 : 1}]} onPress={() => turn == 'Opponent' && userName == opponentName && colorChange(colorOption[3])}></Pressable>
                <Pressable style={[styles.color, {backgroundColor: opponentSelectedColor == colors[4] ? 'gray' : colorOption[4]}, {opacity: opponentSelectedColor == colors[4] ? 0.25 : 1}]} onPress={() => turn == 'Opponent' && userName == opponentName && colorChange(colorOption[4])}></Pressable>
            </View>:
            <View style={[styles.colorRow, turn == 'Opponent' && {opacity: .25}]}>
                <Pressable style={[styles.color, {backgroundColor: ownerSelectedColor == colors[0] ? 'gray' : colorOption[0]}, {opacity: ownerSelectedColor == colors[0] ? 0.25 : 1}]} onPress={() => turn == 'Owner' && userName == ownerName && colorChange(colorOption[0])}></Pressable>
                <Pressable style={[styles.color, {backgroundColor: ownerSelectedColor == colors[1] ? 'gray' : colorOption[1]}, {opacity: ownerSelectedColor == colors[1] ? 0.25 : 1}]} onPress={() => turn == 'Owner' && userName == ownerName && colorChange(colorOption[1])}></Pressable>
                <Pressable style={[styles.color, {backgroundColor: ownerSelectedColor == colors[2] ? 'gray' : colorOption[2]}, {opacity: ownerSelectedColor == colors[2] ? 0.25 : 1}]} onPress={() => turn == 'Owner' && userName == ownerName && colorChange(colorOption[2])}></Pressable>
                <Pressable style={[styles.color, {backgroundColor: ownerSelectedColor == colors[3] ? 'gray' : colorOption[3]}, {opacity: ownerSelectedColor == colors[3] ? 0.25 : 1}]} onPress={() => turn == 'Owner' && userName == ownerName && colorChange(colorOption[3])}></Pressable>
                <Pressable style={[styles.color, {backgroundColor: ownerSelectedColor == colors[4] ? 'gray' : colorOption[4]}, {opacity: ownerSelectedColor == colors[4] ? 0.25 : 1}]} onPress={() => turn == 'Owner' && userName == ownerName && colorChange(colorOption[4])}></Pressable>
            </View>
            }
        </View>
        {userName != opponentName ? <View style={[styles.board, screenWidth > 500 && {maxWidth: screenWidth}]}>
            {colorState.map((sq, index) => {
                return (
                    // <View key={index} style={[styles.square, {backgroundColor: sq.color}]}></View>
                    // 
                    <Animated.View key={index} style={[styles.square, sq.captured && {zIndex: 2}, {backgroundColor: sq.captured && sq.owner == 'Owner' ? ownerColor : sq.captured && sq.owner == 'Opponent' ? opponentColor : colorState[index].color, width: gridItemSize, height: gridItemSize, transform: [
                      {
        ...
        .map((sq, index) => {
            return (
                // <View key={index} style={[styles.square, {backgroundColor: sq.color}]}></View>
                // 
                <Animated.View key={index} style={[styles.square, sq.captured && {zIndex: 2}, {backgroundColor: sq.captured && sq.owner == 'Owner' ? ownerColor : sq.captured && sq.owner == 'Opponent' ? opponentColor : colorState[colorState.length - 1 - index].color, width: gridItemSize, height: gridItemSize, transform: [
                  {
                    scale: squareAnim[squareAnim.length - 1 - index].interpolate({
                      inputRange: [0, 1],
                      outputRange: [1, 1.2], // grow by 20%
                    }),
                  },
       ...
       { userName != opponentName ? <View style={[styles.colorRow, turn == 'Opponent' && {opacity: .25}]}>
            <Pressable style={[styles.color, {backgroundColor: ownerSelectedColor == colors[0] ? 'gray' : colorOption[0]}, {opacity: ownerSelectedColor == colors[0] ? 0.25 : 1}]} onPress={() => turn == 'Owner' && userName == ownerName && colorChange(colorOption[0])}></Pressable>
            <Pressable style={[styles.color, {backgroundColor: ownerSelectedColor == colors[1] ? 'gray' : colorOption[1]}, {opacity: ownerSelectedColor == colors[1] ? 0.25 : 1}]} onPress={() => turn == 'Owner' && userName == ownerName && colorChange(colorOption[1])}></Pressable>
            <Pressable style={[styles.color, {backgroundColor: ownerSelectedColor == colors[2] ? 'gray' : colorOption[2]}, {opacity: ownerSelectedColor == colors[2] ? 0.25 : 1}]} onPress={() => turn == 'Owner' && userName == ownerName && colorChange(colorOption[2])}></Pressable>
            <Pressable style={[styles.color, {backgroundColor: ownerSelectedColor == colors[3] ? 'gray' : colorOption[3]}, {opacity: ownerSelectedColor == colors[3] ? 0.25 : 1}]} onPress={() => turn == 'Owner' && userName == ownerName && colorChange(colorOption[3])}></Pressable>
