...
const Leaderboard = () => {
  ...
  const EmptyRow = () => {
    return (
      <View style={[styles.tableRow, {padding: 10, justifyContent: 'center'}]}>
    ...
    if (gamemode != 'PVP') {
      ...
      const unsubscribe = onSnapshot(q, (querySnapshot) => {
        ...
        querySnapshot.forEach((doc) => {
          ...
          if (!lowestScoresMap.has(boardId) || score < lowestScoresMap.get(boardId).score) {
            if (gamemode == 'Progressive') {
              lowestScoresMap.set(boardId, {
                id: doc.id,
                ...doc.data(),
                score: doc.data().score > 0 ? `+${doc.data().score}` : doc.data().score
              });
            }
            else {
              lowestScoresMap.set(boardId, {
  ...
  return (
    <View style={[styles.container, {backgroundColor: colors.background}]}>
      <Text style={{marginTop: 50, fontSize: 30, color: colors.text}}>Leaderboard</Text>
      <Text style={{marginTop: 25, fontSize: 15, color: colors.text}}>Search Options</Text>
      <View style={styles.searchOptions}>
        {/* { sizeLabel() } */}
        <View style={styles.optionCol}>
          <Text style={{textAlign: 'center', color: colors.text}}>Gamemode</Text>
          <Dropdown 
            style={[styles.dropdown, gamemodeFocus && { borderColor: 'blue'}, {backgroundColor: 'white'}]}
        ...
        {gamemode == 'FreePlay' && <View style={styles.optionCol}>
          <Text style={{textAlign: 'center', color: colors.text}}>Board Size</Text>
          <Dropdown 
            style={[styles.dropdown, sizeFocus && { borderColor: 'blue'}, {backgroundColor: 'white'}]}
            placeholderStyle={styles.placeholderStyle}
            selectedTextStyle={styles.selectedTextStyle}
            inputSearchStyle={styles.inputSearchStyle}
            iconStyle={styles.iconStyle}
        ...
        {gamemode == 'PVP' && <View style={styles.optionCol}>
          <Text style={{textAlign: 'center', color: colors.text}}>Query Options</Text>
          <Dropdown 
            style={[styles.dropdown, queryOptions && { borderColor: 'blue'}, {backgroundColor: 'white'}]}
            placeholderStyle={styles.placeholderStyle}
            selectedTextStyle={styles.selectedTextStyle}
            inputSearchStyle={styles.inputSearchStyle}
            iconStyle={styles.iconStyle}
      ...
      {gamemode != 'PVP' ? <View style={styles.table}>
        <View style={[styles.tableRow, {backgroundColor: colors.tableRow}]}>
          <View style={[styles.tableCol, {width: '15%', maxWidth: '15%'}]}>
            <Text style={{textAlign: 'center', padding: 5, color: colors.text}}>Rank</Text>
          </View>
          <View style={[styles.tableCol, {width: '45%', maxWidth: '45%'}]}>
            <Text style={{textAlign: 'center', padding: 5, color: colors.text}}>User</Text>
          </View>
          ...
          <View style={[styles.tableCol, {width: '20%', maxWidth: '20%'}]}>
            <Text style={{textAlign: 'center', padding: 5, color: colors.text}}>Score</Text>
          </View>
          {/* <View style={[styles.tableCol, ]}>
            <Text style={{textAlign: 'center', padding: 5}}>Size</Text>
          </View> */}
          <View style={[styles.tableCol, {width: '20%', maxWidth: '20%'}]}>
            <Text style={{textAlign: 'center', padding: 5, color: colors.text}}>Link</Text>
          </View>
        </View>
        <FlatList 
        data={scores}
        ...
        renderItem={({ item, index }) => (
          <View style={[styles.tableRow, {backgroundColor: colors.tableRow}]}>
              ...
              <View style={[styles.tableCol, {width: '25%', maxWidth: '50%', alignItems: 'center', padding: 15}]}>
                <Text style={{textAlign: 'center', padding: 5, color: colors.text}}>{item.createdBy}</Text>
              </View>
              ...
              <View style={[styles.tableCol, {width: '20%', maxWidth: '20%', alignItems: 'center', padding: 15}]}>
                <Text style={{textAlign: 'center', padding: 5, color: gamemode == 'Progressive' && item.score < 0 ? 'green' : gamemode == 'Progressive' && item.score > 0 ? 'red' : colors.text}}>{item.score}</Text>
              </View>
              {/* <View style={[styles.tableCol]}>
                <Text style={{textAlign: 'center', padding: 5}}>Medium</Text>
              </View> */}
              <View style={[styles.tableCol, {width: '20%', maxWidth: '20%', justifyContent: 'center'}]}>
              {/* <Pressable style={{padding: 5}} onPress={() => handleLink(item.id, item.data.size)}> */}
                <Pressable style={{padding: 5}} onPress={() => handleLink(item.boardId)}>
      ...
      <View style={styles.table}>
        <View style={[styles.tableRow, {backgroundColor: colors.tableRow}]}>
          ...
          <View style={[styles.tableCol, {width: '42.5%', maxWidth: '42.5%',}]}>
            <Text style={{textAlign: 'center', padding: 5, color: colors.text}}>Username</Text>
          </View>
          <View style={[styles.tableCol, {width: '42.5%', maxWidth: '42.5%',}]}>
            <Text style={{textAlign: 'center', padding: 5, color: colors.text}}>{queryOptionState}</Text>
          </View>
        </View>
        <FlatList 
        data={pvpResults}
