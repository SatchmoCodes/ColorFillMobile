...
const BoardInfo = ({ navigation }) => {
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        const q = query(collection(db, 'Scores'), where('boardId', '==', docId), orderBy('score', 'asc'), orderBy('createdAt', 'asc'))
            ...
            if (!querySnapshot.empty) {
                ...
                querySnapshot.forEach((doc) => {
                    if (gamemode == 'Progressive') {
                        console.log('true')
                        scoreListArr.push({...doc.data(), score: doc.data().score > 0 ? `+${doc.data().score}` : doc.data().score})
                    }
                    else {
    ...
    async function initialLoad() {
        const creatorQuery = query(collection(db, 'Scores'), where('boardId', '==', docId), orderBy('createdAt', 'asc'))
        console.log(docId)
    ...
    const LoadedView = ({ createdBy, highScore, scoreList }) => {
        return (
            <>
              <Text style={{textAlign: 'center',fontSize: 30, color: colors.text}}>Board Info</Text>
              <Pressable style={styles.button} onPress={() => loadBoard(highScore.id, highScore.data.size)}>
              ...
              <Text style={{textAlign: 'center', fontSize: 20, marginBottom: 10, color: colors.text}}>Created By: {createdBy.createdBy}</Text>
              <Text style={{textAlign: 'center', fontSize: 20, marginBottom: 10, color: colors.text}}>Board Size: {highScore.data.size}</Text>
              <Text style={{textAlign: 'center', fontSize: 20, marginBottom: 10, color: colors.text}}>Best Score: {highScore.data.createdBy} ({highScore.data.score})</Text>
              <Text style={{textAlign: 'center', fontSize: 20, marginTop: 20, marginBottom: 20, color: colors.text}}>All Scores</Text>
              <View style={styles.table}>
                <View style={[styles.tableRow, {backgroundColor: colors.tableRow}]}>
                    <View style={[styles.tableCol, {padding: 15, alignItems: 'center', width: '20%', maxWidth: '20%'}]}>
                        <Text style={[{color: colors.text}]}>Rank</Text>
                    </View>
                    <View style={[styles.tableCol, {padding: 15, alignItems: 'center', width: '40%', maxWidth: '40%'}]}>
  ...
  return (
    <View style={[styles.container, {backgroundColor: colors.background}]}>
        {!loading ?
        <LoadedView createdBy={createdBy} highScore={highScore} scoreList={scoreList}></LoadedView>
        :
